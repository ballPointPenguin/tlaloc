...
When asked to update and publish the page, do these steps in order:
...
3. Call git_commit_and_push with a concise commit message.
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "Stage index.html, commit, and push to origin/main, publishing to GitHub Pages. "
            "Call this after update_weather_content succeeds. "
            "Write a commit message summarising the conditions and location."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "commit_message": {
                    "type": "string",
                    "description": "Git commit message summarising the weather update",
                },
            },
...
def git_commit_and_push(commit_message: str) -> dict:
    def run(*args):
        return subprocess.run(
            list(args), capture_output=True, text=True, cwd=str(REPO_DIR)
        )

    stage = run("git", "add", "index.html")
    if stage.returncode != 0:
        return {"success": False, "step": "add", "error": stage.stderr.strip()}
    ...
    if commit.returncode != 0:
        ...
        return {"success": False, "step": "commit", "error": commit.stderr.strip()}

    push = run("git", "push", "origin", "main")
    if push.returncode != 0:
        return {"success": False, "step": "push", "error": push.stderr.strip()}

    return {
        "success": True,
        "commit_message": commit_message,
        "output": push.stderr.strip() or push.stdout.strip(),
    }
...
def run_tool(name: str, inputs: dict) -> dict:
    ...
    if name == "git_commit_and_push":
        return git_commit_and_push(**inputs)
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "update the Tlaloc weather page, and publish to GitHub Pages."
            ),
        }
    ]
